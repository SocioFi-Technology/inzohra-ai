...
type Props = {
  ...
  /**
   * When true, the component renders without the outer <aside> wrapper
   * and without a collapsible header (the tab strip already labels it).
   * Used when embedded in a tabbed panel.
   */
  embedded?: boolean;
};
      ...
      <div className="divide-y divide-gray-100">
        {FIELD_LABELS.map(([key, label]) => {
          ...
          return (
            <div
              key={key}
              className={["px-4 py-2.5", isActive ? "bg-blue-50" : ""].join(" ")}
            >
              ...
              <div className="flex items-start justify-between gap-2">
                ...
                {hasBbox && (
                  <button
                    onClick={() =>
                      onShowSource(fieldId, field.bbox as [number, number, number, number])
                    }
        ...
        <div className="px-4 py-2.5 bg-gray-50">
          <span className="text-xs text-gray-400">
            Entity confidence: {Math.round(entity.confidence * 100)}% · {entity.source_track}
          </span>
        </div>
      </div>
    </>
...
export function TitleBlockPanel({
  ...
  // ---- Embedded mode (inside tabbed panel, no outer aside) ----
  if (embedded) {
    if (!entity) {
      return (
  ...
  // ---- Standalone mode (own aside wrapper with collapse) ----
  if (!entity) {
    return (
